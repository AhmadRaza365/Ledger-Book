...
function UpdateOrderPage() {
  ...
  useEffect(() => {
    if (orderData) {
      setSelectedCustomer(
        allCustomers.find(
          (customer) => customer.uuid === orderData?.customerID
        ) ?? {
          uuid: orderData?.customerID,
          name: orderData?.customerName,
          phoneNo: orderData?.customerPhoneNo,
      ...
      setDueDate(orderData.dueDate);
      setDeliveryDate(orderData.delivery.deliveryDate ?? "");
      setDeliveryPersonName(orderData.delivery.deliveryPersonName ?? "");
      setDeliveryPersonPhoneNo(orderData.delivery.deliveryPersonPhoneNo ?? []);
      setDeliveryAddress(orderData.delivery.deliveryAddress ?? "");
      setDeliveryTruckNo(orderData.delivery.deliveryTruckNo ?? "");
      setDeliveryStatus(orderData.delivery.deliveryStatus ?? "");
      setProducts(orderData.products ?? []);
      setOrderPayments(orderData.orderPayments ?? []);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [orderData]);
    ...
    if (
      ...
      try {
        ...
        await UpdateOrder({
          ...
          orderDate: orderDate,
          dueDate: dueDate ?? "",
          orderNo: orderData?.orderNo ?? `${totalOrders + 1}`,
          uuid: orderData?.uuid ?? uuidv4(),
